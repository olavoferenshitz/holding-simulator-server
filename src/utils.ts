...
export function formatCurrencyToNumber(currencyValue: string): number {
  return Number(
    currencyValue.substring(0, currencyValue.length - 3).replace(/\D/g, "")
  );
}

export function setCreatedAtToBrazilTimeZone(date: Date): Date {
  const parsedDate = new Date(date);

  parsedDate.setUTCHours(parsedDate.getUTCHours() - 3);
